...
typedef struct pin_pair {
	uint32_t input;
	uint32_t response;
} pin_pair_t;

pin_pair_t A = { TEST_A, RESPONSE_A }, B = { TEST_B, RESPONSE_B }, C = { TEST_C, RESPONSE_C };

//static void vBasicTask(void *pvParameters);
static void toggleLed0(void *pvParameters);
static void toggleLed1(void *pvParameters);
/* Parameter is a struct containing input and output pins */
static void reactToButterfly(void *pvParameters);
static void wasteTime(void *pvParameters);
...
int main()
{
	...
	// Assignment A
	xTaskCreate( toggleLed0, (signed char * ) "LED0", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate( toggleLed1, (signed char * ) "LED1", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
